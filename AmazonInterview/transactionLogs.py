...
def findFrauds(logs, threshold):
    counter = Counter()
    for log in logs:
        split_log = log[0].split(" ")
        id_1, id_2 = split_log[0], split_log[1]
        if id_1 == id_2:
            counter[id_1] += 1
        else:
            counter[id_1] += 1
            counter[id_2] += 1
    return sorted([key for key, value in counter.items() if value >= threshold], reverse=True)
...
if __name__ == "__main__":
    ...
    threshold = 3
    print(findFrauds(transaction_logs, threshold))